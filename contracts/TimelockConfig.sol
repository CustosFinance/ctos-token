...
interface ITimelockConfig {
    ...
    function calculateConfigID(string memory name)
        external
        pure
...
contract TimelockConfig is ITimelockConfig {
    ...
    function confirmChange(bytes32 configID) external override onlyAdmin {
        ...
        require(
            block.timestamp >=
                _pending[configID].timestamp + _config[TIMELOCK_CONFIG_ID],
            "too early"
        );
    ...
    function requestChange(bytes32 configID, uint256 value)
        external
        override
        onlyAdmin
    {
    ...
    function isConfig(bytes32 configID)
        public
        view
        override
        returns (bool status)
    ...
    function isPending(bytes32 configID)
        public
        view
        override
        returns (bool status)
